...
<title>无敌汽车网论坛-无敌汽车网-汽车点滴 尽在无敌-中国汽车文化发源地</title>
<style type="text/css">
...
list($modi_uid, $modi_email) 			= explode("\t", uc_authcode($_COOKIE['modie_auth'], 'DECODE'));
	...
	$sqllogin_modi="SELECT * FROM `pre_common_member` where `uid`='$_COOKIE[modi_user]'";
...
?>
                    ...
                    <span class="nickname"><a href="http://bbs.modiauto.com.cn" target="_blank"><?php echo $rowlogin_modi[username];?></a></span>
                    

<?php }else{ ?>
                   
<form action="" method="post" class="modiuser_login" style="display:none;">
		...
		if($rowlogin_modi[modi_id]){ 
                ...
                if($resultv=='true'){
                    ...
                    exit();
                        ...
                        //echo 'error';
                        echo '<script>alert("无敌会员激活失败,请联系管理员QQ：624734")</script>';
