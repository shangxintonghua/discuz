...
function checkArc(aid){
	var qstr=getCheckboxItem();
	if(aid==0) aid = getOneItem();
    form2.dopost.value = "check";
    form2.aid.value = aid;
    form2.qstr.value = qstr;
    form2.submit();
}
function pubArc(aid){
    ...
    var qstr=getCheckboxItem();
    var isremote=0;
    if(document.form2.isremote.checked) isremote=1;
    if(aid==0) aid = getOneItem();
    if (isremote == 0)
        form2.dopost.value = "prepub";
    else
        form2.dopost.value = "pub";
    form2.aid.value = aid;
    form2.qstr.value = qstr;
    form2.submit();
}
...
function moveLevel(e, obj){
	...
	LoadQuickDiv(e, 'archives_do.php?dopost=upLevel&qstr='+qstr+'&rnd='+Math.random(), 'upLevel', '450px', '180px');
	ChangeFullDiv('show');
}
function moveModel(e, obj){
	console.log(qstr);
	var qstr=getCheckboxItem();
	if(qstr=='')
	{
	...
	LoadQuickDiv(e, 'archives_do.php?dopost=upModel&qstr='+qstr+'&rnd='+Math.random(), 'upModel', '450px', '180px');
	ChangeFullDiv('show');
}
function adArc(aid){
	var qstr=getCheckboxItem();
